...
Writing $A$ for the area of the region shared by the two meshes, $n_{\mathrm{bbox}}$ for the number of \MPAS cells whose centers fall inside it, and $n_{\mathrm{cells}}$ for the number of \ROMS cells, the characteristic lengths of the two meshes are $\hM=\sqrt{A/n_{\mathrm{bbox}}}$ and $\hR=\sqrt{A/n_{\mathrm{cells}}}$, so that each is the side of the square cell that would tile the shared region with the given cell count. Their ratio $\hRatio=\hR/\hM=\sqrt{n_{\mathrm{bbox}}/n_{\mathrm{cells}}}$ depends only on the two counts, so it is independent of how the shared region is delimited. Four nested \ROMS meshes, r1 (coarsest) through r4 (finest), are used in each region. Their inventory is given in Table~\ref{conv:tab:grid-inventory}.

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{3pt}
\caption{\ROMS uniform mesh refinements: r1 (coarsest) to r4 (finest).}
\label{conv:tab:grid-inventory}
\begin{tabular}{llrrrr}
\toprule
Region & Label & $\eta_\rho\times\xi_\rho$ & Cells$^{\dagger}$ & $\hR$ (km) & $\hRatio$ \\
\midrule
\multirow{4}{*}{Gulf Stream} & r1 & $38\times112$ & $4107$ & $14.70$ & $2.12$ \\
& r2 & $114\times334$ & $37629$ & $4.92$ & $0.71$ \\
& r3 & $341\times1001$ & $340000$ & $1.64$ & $0.24$ \\
& r4 & $1023\times3003$ & $3068044$ & $0.55$ & $0.08$ \\
\midrule
\multirow{4}{*}{Bay of Bengal} & r1 & $24\times40$ & $897$ & $36.05$ & $3.21$ \\
& r2 & $74\times120$ & $8687$ & $12.02$ & $1.06$ \\
& r3 & $222\times362$ & $79781$ & $4.01$ & $0.35$ \\
& r4 & $666\times1086$ & $721525$ & $1.34$ & $0.12$ \\
\bottomrule
\end{tabular}
...
The \MPAS mesh in the Gulf Stream region has a local refinement with characteristic length of $\hM=\SI{6.89}{\kilo\metre}$, whereas over the Bay of Bengal region, the \MPAS mesh has a coarser characteristic length of $\hM=\SI{11.28}{\kilo\metre}$. The same labels therefore denote different absolute characteristic lengths but comparable relative refinements. For the regional cases demonstrated in this work, we have chosen \ROMS meshes with r3 labels for the Gulf Stream and Bay of Bengal with $\hRatio$ of $0.24$ and $0.35$ respectively.



\subsubsection{One-dimensional axial refinement study}\label{conv:sec:axial-refinement}

Figure~\ref{conv:fig:physical-1d-workflow} isolates the axial factor under joint refinement of the \MPAS and \ROMS layers. It is a reduced representation of the geometry in Fig.~\ref{conv:fig:coord-mismatch} where the selected nonuniform \MPAS columns retain the physical $z^*$ layer-thickness patterns of the Gulf Stream and Bay of Bengal \MPAS meshes, while the \ROMS layer interfaces are regenerated from the same terrain-following stretching families used by the regional grids. The 1-D convergence study utilizes a domain of physical length scale matching bathymetry in the chosen domains, with the layer thicknesses from the original \MPAS setup in these domains, and with corresponding \ROMS columns embedded in the resolved \MPAS column [SSH, Bathymetry] range based on the consistent stretching functions used in real cases. In this study, we refine both the source \MPAS column uniformly, and recompute the refined \ROMS terrain following levels accordingly. This sequence of refined columns provides a realistic setup to identify and evaluate the error propagation behavior of the vertical remaps, while removing the influence of horizontal remap effects.

The two axial operators are compared here because they sit at opposite ends of the same spectrum as the 2D horizontal remapping workflow. The conservative axial projection, which is the one used for the three-dimensional transfers reported in this manuscript, takes \MPAS cell averages to \ROMS cell averages and preserves each \ROMS layer integral based on an overlap mesh computation. The PCHIP, non-conservative, quasi-interpolant instead reconstructs the profile itself, sampled at the \MPAS layer interfaces and evaluated at the \ROMS interfaces with no cell-average projection on either side, to attain higher order convergence on smooth data. Each operator is also run on a uniform subdivision of the same physical column at every axial cell count, as a control on the terrain-following interfaces. Due to the non-constant embedding of the \MPAS and \ROMS meshes, the adjacent-level refinements can result in a non-uniform, monotonically converging sequence, that require least-squares slopes of $\log L_2$ against $\log \hR$ for determining the convergence rate.

Over the tested columns and the smooth sinusoidal profile, the conservative projection converges at first order in the mean layer thickness, consistent with the $\mathcal{O}(h_z)$ reference drawn in the figure, with measured slopes of $\approx 1.05$ for both domains. The PCHIP interpolant converges at a faster rate of $\approx 2.5$, but falls short of the $\mathcal{O}(h_z^{3})$ that an unlimited cubic-Hermite reconstruction would attain. This is due to the fact that the monotonicity limiter activated in the reconstruction of strong gradients can degrade the rate to $\mathcal{O}(h_z^{2.5})$. As a separate experiment, we repeated the convergence verification for the same sequence of uniformly subdivided \ROMS grid, in place of the terrain-following one, to determine that the rates are identical. This verification implies that the choice of geopotential or terrain-following vertical coordinate system itself does not bias the error propagation.

%The two operators have distinct output semantics, the conservative one evaluated from \ROMS layer integral residuals and the interpolant from interface point values, so the comparison is between two workflow contracts rather than between interchangeable discrete $L_2$ measures, and it is limited to the physical columns, stretching families, profile, and refinement range tested here.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Figures/conv_1d_convergence.png}
\caption{One-dimensional convergence of the two axial operators, matching support. Dotted line: $\mathcal{O}(h)$ reference.}
...
\TODOc{Rerun more experiments to determine cost for refined GS and BoB cases}
...
\TODOc{Also need to modify code to eliminate all I/O etc in timing}
...
When the projected bathymetry in the \ROMS model changes rapidly in adjacent cells, the model can generate sigma-coordinate pressure-gradient errors over steep slopes~\citep{haney1991pressure,mellor1994pressure}. We condition it with a one-ring \emph{log-depth} averaging smoother. Writing $H_i$ for the depth (bathymetry) of \ROMS cell $i$, $q_i=\log H_i$ for its logarithm, $\mathcal{A}(i)$ for the set of edge-neighbours of that cell, and $m$ for the iteration counter, each sweep replaces the \emph{log-depth} of a cell by the mean over its one-ring neighbourhood, similar to a Laplacian smoother in the context of mesh optimization workflows. The iterative sequence can be wrriten as 
%
\begin{equation}
 q_i^{(m+1)}=\frac{1}{|\mathcal{A}(i)|}\sum_{i'\in\mathcal{A}(i)}q_{i'}^{(m)},\qquad H_i^{(m+1)}=\exp q_i^{(m+1)},
 \label{conv:eq:onering}
\end{equation}
with clipping to the \MPAS range. Operating on $\log H$ rather than $H$ makes the smoother act on relative rather than absolute depth differences, which results in the sweeps having a comparable effect on the shelf and in the deep ocean regions. Land cells are typically excluded from these neighbourhood sums and the land masks are preserved throughout the process. Smoothing by Eq.~\eqref{conv:eq:onering} stops once the steepest remaining neighbour contrast satisfies
\begin{equation}
 r_{\max}^{(m)}=\max_i\max_{i'\in\mathcal{A}(i)}\frac{q_i^{(m)}-q_{i'}^{(m)}}{q_i^{(m)}+q_{i'}^{(m)}}<0.05.
 \label{conv:eq:slope-stop}
\end{equation}
Evaluated on the same log-depths, this test plays the role of the $r$-factor conventionally used to condition terrain-following grids~\citep{sikiric2009smoothing}. The results from applying the bathymetry smoothing and its effect on the stability of the \ROMS model simulation is described in Fig.~\ref{conv:fig:smoothing}.
\Review{Kyle: Confirm whether the BoB stability issues occurred without smoothing in the newer runs. I do not remember if the figure is from an older experiment}
...
Limiting the logarithm of the bathymetry changes in neighborhoods to be less than 5\% kept the \ROMS simulations stable in the Bay of Bengal domain. This smoothing process bounded the maximal remapping error and its propagation during simulation, which removed a warm temperature anomaly between 100m and 200m near masked coastlines in the unsmoothed case. This procedure is vital in regional applications where coastal or estuarine topography is inadequately resolved by the \MPAS global model, which forces us to blend with the native \ROMS bathymetry and to retain the \ROMS land mask as needed. 

The Chesapeake Bay domain is the clearest example showcasing this issue. Here, the \MPAS mesh cannot represent the estuarine geometry at all, and so the boundary and initial fields there must be prescribed or blended.
\TODOc{Kyle: Talk more about bathymetry blending, smoothing, initial/forcing condition extrapolation etc for Chesapeake Bay case}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Figures/saltCompare_ChesBay_frames_0055.png}
\caption{Chesapeake Bay domain showing evolution of salinity at day=55.}
\label{conv:fig:chesapeake-bay-salinity}
\end{figure}
\FloatBarrier
