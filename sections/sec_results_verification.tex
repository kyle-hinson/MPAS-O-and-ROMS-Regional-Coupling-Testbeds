...
The Gulf Stream \MPAS mesh is locally refined to a characteristic length of $\hM=\SI{6.89}{\kilo\metre}$ over the shared region, whereas the coarser Bay of Bengal \MPAS mesh gives $\hM=\SI{11.28}{\kilo\metre}$. The same labels therefore denote different absolute characteristic lengths but comparable relative refinement. The r3 meshes are the ones used for the Gulf Stream and Bay of Bengal regional applications of \S\ref{conv:sec:bathysmooth}, so those applications sit at $\hRatio=0.24$ and $0.35$ respectively.



\subsubsection{One-dimensional axial refinement study}\label{conv:sec:axial-refinement}

Figure~\ref{conv:fig:physical-1d-workflow} isolates the axial factor under joint refinement of the \MPAS and \ROMS layers. It is a reduced representation of the geometry in Fig.~\ref{conv:fig:coord-mismatch} where the selected nonuniform \MPAS columns retain the physical $z^*$ layer-thickness patterns of the Gulf Stream and Bay of Bengal \MPAS meshes, while the \ROMS layer interfaces are regenerated from the same terrain-following stretching families used by the regional grids. Two support cases are exercised, a matching case in which the \ROMS column is embedded in the resolved \MPAS column [SSH, Bathymetry] range, and a fixed-bottom extrapolation case in which it extends slightly beyond that range due to the stair stepping of the bathymetry on the finer \ROMS cells. The figure reports the matching case, and the extrapolation case is discussed below. Thus the one-dimensional sequence retains the vertical-coordinate mismatch, and refinement mechanism of the regional cases while removing influence of horizontal remap effects.

The two axial operators are compared here because they sit at opposite ends of the same trade. The conservative axial projection, which is the one used for the three-dimensional transfers reported in this manuscript, takes \MPAS cell averages to \ROMS cell averages and reproduces each \ROMS layer integral. The PCHIP interpolant instead reproduces the profile itself, sampled at the \MPAS layer interfaces and evaluated at the \ROMS interfaces with no cell-average projection on either side, so it attains higher order on smooth data but conserves nothing. Each operator is also run on a uniform subdivision of the same physical column at every axial cell count, as a control on the terrain-following interfaces. Because the adjacent-level orders are not clean over this sequence, the rates quoted are least-squares slopes of $\log L_2$ against $\log \hR$ over all six levels.

Over the tested columns and the smooth sinusoidal profile, the conservative projection converges at first order in the mean layer thickness, consistent with the $\mathcal{O}(h)$ reference drawn in the figure, with measured slopes of $1.04$ in the Gulf Stream column and $1.07$ in the Bay of Bengal column. The PCHIP interpolant converges faster, at $2.72$ and $2.26$ respectively, but falls short of the third order that an unlimited cubic Hermite reconstruction would attain, because the monotonicity limiter activates at the interior extrema of the test profile and holds the global rate near five halves. Repeating the same sequence on a uniform subdivision of each column, in place of the terrain-following interfaces, changes the conservative slopes to $1.04$ and $1.03$ and the interpolant slopes to $2.63$ and $2.42$, differences smaller than the spread between the two regions, so the vertical coordinate itself is not what limits either operator.

The fixed-bottom extrapolation cases behave the same way over the coarse part of the sequence and then flatten, because the nearest-neighbour continuation below the \MPAS bottom leaves a residual that \ROMS refinement cannot reduce, so their fitted slopes measure the overhang closure rather than either operator and are omitted from the figure. The two operators have distinct output semantics, the conservative one evaluated from \ROMS layer integral residuals and the interpolant from interface point values, so the comparison is between two workflow contracts rather than between interchangeable discrete $L_2$ measures, and it is limited to the physical columns, stretching families, profile, and refinement range tested here.
